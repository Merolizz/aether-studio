// SHA-256 implementation for HardwareID hashing
// Uses OpenSSL if available, otherwise uses built-in implementation

#include "../../include/aether/SHA256.h"
#ifdef AETHER_OPENSSL_ENABLED
...
std::string SHA256::hash(const std::string &input) {
#ifdef AETHER_OPENSSL_ENABLED
  // Use OpenSSL if available (faster and more secure)
  ...
  // sha256.update(reinterpret_cast<const uint8_t*>(input.c_str()),
  // input.length()); sha256.final(hash);
