    ...
    if (hardwareOrchestrator.getAvailableGPUs().size() > 0) {
        ...
        if (selectedGPU.vulkanDevice != VK_NULL_HANDLE) {
            ...
            if (queueFamilyCount > 0) {
                ...
                vkGetPhysicalDeviceQueueFamilyProperties(selectedGPU.vulkanDevice, &count, queueFamilies.data());
            }
...
bool VulkanRenderer::createSwapchain(Window* window) {
    ...
    if (presentModeCount > 0) {
        ...
        uint32_t count = presentModeCount;
...
bool VulkanRenderer::beginFrame() {
    if (m_swapchainImages.empty() || m_currentFrame >= m_inFlightFences.size()) {
        return false;
    }
    ...
    if (imageIndex >= m_swapchainImages.size()) {
        std::cerr << "Acquired image index out of range" << std::endl;
        return false;
    }
